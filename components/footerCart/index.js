...
export default function FooterCart(props) {
  ...
  return (
    <Box className='w-full footer footer-cart'>
      <div className='my-2 p-4  flex items-center justify-between cursor-pointer' onClick={toggleDrawer(true)}>
        <div >
        ...
        <p className={coupon != null? 'color-C5A153':''}>  {coupon != null? coupon:'Chọn hoặc nhập mã'} <ChevronRightIcon /></p>
      </div>
      <div className='mb-4'>
        <Button  style={{background:'#23432E',borderRadius: 8,padding:15,margin:'0 15px',width:'calc(100% - 30px)'}} variant="contained"><span className=' text-base font-semibold'>Xác nhận đơn</span></Button> 
      </div>
      ...
      <Root>
        ...
        >
            <div className={styles.rulesMain}>
                <div className="coupon-form--title">Mã áp dụng <CloseIcon onClick={toggleDrawer(false)}/></div> 
                <div className='coupon-form flex'>
                  <input type="text" value={coupon} disabled placeholder='Nhập mã'/>
                  <Button onClick={toggleDrawer(false)} disabled={coupon != null? '':'disabled'} variant="contained">Áp dụng</Button>
                </div>
                <div className="coupon-form--content">
                  { listCoupon.map(function(d,idx){
                    return(
                      <button key={idx} className={d.active == 2? 'btn-coupon chddk':'btn-coupon'} disabled={d.active == 1?'':'disabled'} onClick={e=> setCoupon(d.key)}>
                        <div className='btn-coupon--body'>
                          <div>
                            ...
                            <div className='btn-coupon-center'>
                              <h3>{d.title}</h3>
                              <p>Dùng được {d.timesOfUse} lần</p>
                              <span>{d.expiry}</span>
                            </div>
                            <div className='btn-coupon-right'>
                              {d.active == 1 && <>
                                <p>Đã dùng</p>
                                <p><strong>{d.timesUsed}/{d.timesOfUse}</strong> lần</p>
                              </>}
                              {d.active == 2 && <>
                ...
                <Button onClick={toggleDrawer(false)}  style={{background:'#23432E',borderRadius: 8,padding:15,margin:'0 15px',width:'calc(100% - 30px)'}} variant="contained"><span className=' text-base font-semibold'>Đồng ý</span></Button> 
            </div>
